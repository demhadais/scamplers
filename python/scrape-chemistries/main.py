...
def find_ranger_dir(lib_dir: Path) -> Path | None:
    ...
    if not cellranger_paths:
        ...
        if (
            lib_name.startswith("sc") or lib_name.startswith("25")
        ) and not citeseq_count:
...
def read_cmdline(
    ...
    if cmdline is None and (
        lib_dir.lower().startswith("sc") or lib_dir.lower().startswith("25e")
    ):
...
def read_chemistry_name_from_summary_json(ranger_dir: Path) -> str | None:
    ...
    if not chemistry_names:
        print(f"no summary*.json found for {ranger_dir.parent}")
        return None
...
def listify_chemistry_cmdlines(
    chemistry_defs: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    for chem_def in chemistry_defs:
        chem_def["cmdlines"] = list(chem_def["cmdlines"])

    return chemistry_defs


def parse_lib_dir(lib_dir: Path) -> tuple[str, str, str] | None:
    ...
    if not chemistry_name:
        if lib_dir.name.lower().startswith("sc") or lib_dir.name.lower().startswith(
            "25e"
        ):
            print(f"couldnt find chemistry anywhere for {lib_dir}")

        return None

    return cmdline, chemistry_name, lib_dir.name


def parse_tenx_dir(tenx_dir: Path) -> list[tuple[str, str, str]]:
    res = (parse_lib_dir(lib_dir) for lib_dir in tenx_dir.iterdir())

    return [tup for tup in res if tup]


def parse_lab_dir(lab_dir: Path) -> list[tuple[str, str, str]] | None:
    ...
    return [
        (cmdline, chemistry_name, lib_dir)
        for tenx_dir in tenx_dirs
        for cmdline, chemistry_name, lib_dir in parse_tenx_dir(tenx_dir)
    ]
...
def main(
    ...
    for lab_dir in Path(delivery_dir).iterdir():
        ...
        for cmdline, chemistry_name, lib_dir in cmdline_chemistry_pairs:
    ...
    chemistry_defs = listify_chemistry_cmdlines(chemistry_defs)
