...
use super::WriteToDb;
use crate::{db::model::person::grant_roles_to_user, result::ScamplersResult};
...
impl NewAdmin {
    pub(super) async fn write(self, db_conn: &mut AsyncPgConnection) -> ScamplersResult<()> {
        ...
        diesel::select(grant_roles_to_user(
            created_user.person.core.summary.handle.id.to_string(),
